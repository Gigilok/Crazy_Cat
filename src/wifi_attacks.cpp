...
void startEvilTwin(uint8_t networkIndex) {
    ...
    // Ativa o captive portal (usa o server(80) do wifi_portal.cpp)
    // NÃO modifica o wifi_portal.cpp - apenas chama startPortal
    extern void startPortal(const char* ssid);
    startPortal(cloneSSID);
    Serial.printf("[EvilTwin] Portal ativo para SSID: %s\n", cloneSSID);
}

void stopEvilTwin() {
    if (!evilTwinActive && !deauthActive) return; 
    
    // Para o captive portal junto
...
// DRONE JAMMER — carrier 915MHz via CC1101
extern bool cc1101Initialized;
extern void cc1101SetFrequency(uint32_t);
